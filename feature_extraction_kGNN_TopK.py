...
# save figure
plt.savefig(os.path.join(data_path, 'plots', 'tsne_kGNN_TopK.png'))
...
def show_UMAP_embedding(features, labels_list, legend=['Fake', 'True']):
    ...
    plt.xlabel('Component 1')
    plt.ylabel('Component 2')
    plt.title('UMAP embedding of cascade-wise features produced \n at last pooling layer')
    plt.show()


# show results
show_UMAP_embedding(features_data, features_labels)

# save figure
plt.savefig(os.path.join(data_path, 'plots', 'umap_kGNN_TopK.png'))
